...
internal class EndCondition : IEndCondition
{
    ...
    [Description("If must_remain_only_one_team is false here you can decide which vanilla teams are allowed to be alive and the maximum number of members that they can have. Leave both this and remaining_custom_teams empty to allow every team\n# You don't need to include here the role's own team")]
    ...
    [Description("The same as remaining_teams but for the UCR 'CustomTeam' custom module - the key is the team name (case insensitive)")]
    public Dictionary<string, int> RemainingCustomTeams { get; set; } = new();

    [Description("Vanilla teams that are completely ignored while evaluating this condition (they can neither block nor end the round)")]
    public List<Team> IgnoredTeams { get; set; } = [Team.OtherAlive];

    [Description("Here you can decide how many people of the other teams are needed to keep the round going (this will be effective if must_remain_only_one_team is false)")]
